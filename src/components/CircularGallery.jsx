  ...
  if (!fontUrl) {
    if (document.fonts && document.fonts.load) {
      try {
        ...
        // Keep the provided font string if the host font is not ready.
      }
    }
    return font;
  }
  ...
  try {
    ...
    if (document.fonts && document.fonts.load) {
      try {
        ...
        // Rendering can still fall back gracefully.
      }
    }
    return resolved;
  } catch (error) {
...
class Title {
  constructor({ gl, plane, text, textColor = '#ffffff', font = DEFAULT_FONT }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
    this.text = text;
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          float planeAspect = max(uPlaneSizes.x / max(uPlaneSizes.y, 0.001), 0.001);
          float imageAspect = max(uImageSizes.x / max(uImageSizes.y, 0.001), 0.001);
          ...
          vec4 coverColor = texture2D(tMap, coverUv);

          vec2 containScale = imageAspect > planeAspect
            ? vec2(1.0, planeAspect / imageAspect)
            : vec2(imageAspect / planeAspect, 1.0);
          vec2 containUv = (vUv - 0.5) / containScale + 0.5;
          float inside = step(0.0, containUv.x) * step(containUv.x, 1.0) * step(0.0, containUv.y) * step(containUv.y, 1.0);
          vec4 fullColor = texture2D(tMap, clamp(containUv, 0.0, 1.0));
          vec4 color = vec4(mix(coverColor.rgb * 0.42, fullColor.rgb, inside), mix(0.86, fullColor.a, inside));
...
class GalleryApp {
  ...
  onPointerMove(event) {
    ...
    this.lastPointerX = event.clientX;
    this.scroll.target = this.scroll.position + distance * (this.scrollSpeed * 0.025);
  }
  ...
  onPointerUp(event) {
    ...
    if (!this.didDrag) {
      this.selectNearest(event.clientX);
      return;
    }
  ...
  selectNearest(clientX) {
    ...
    const pointerWorldX =
      typeof clientX === 'number' ? ((clientX - rect.left) / rect.width - 0.5) * this.viewport.width : 0;
    const nearest = this.medias.reduce((closest, media) => {
      if (!closest) return media;
      return Math.abs(media.plane.position.x - pointerWorldX) < Math.abs(closest.plane.position.x - pointerWorldX)
        ? media
        : closest;
    }, null);
    if (!nearest) return;
    this.onItemClick(nearest.index % this.itemsLength);
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const start = async () => {
      const resolvedFont = showText ? await resolveFont(font, fontUrl) : font;
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, showText, onItemClick]);
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Drag horizontally or use arrow keys to browse."
    />
  );
}
